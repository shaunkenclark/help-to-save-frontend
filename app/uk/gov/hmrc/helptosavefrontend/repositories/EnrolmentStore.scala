...
import uk.gov.hmrc.helptosavefrontend.repositories.EnrolmentStore.{Enrolled, NotEnrolled, Status}
import uk.gov.hmrc.helptosavefrontend.repositories.MongoEnrolmentStore.EnrolmentData
...
trait EnrolmentStore {
  ...
  def put(nino: NINO, imtpNeedsUpdate: Boolean): EitherT[Future,String,Unit]
...
object EnrolmentStore {
  ...
  sealed trait Status {
    def fold[T](ifNotEnrolled: ⇒ T, ifEnrolled: Boolean ⇒ T): T = this match {
      case e: Enrolled  ⇒ ifEnrolled(e.imtpNeedsUpdate)
      case NotEnrolled  ⇒ ifNotEnrolled
    }
  }

  case class Enrolled(imtpNeedsUpdate: Boolean) extends Status
...
class MongoEnrolmentStore @Inject()(mongo: ReactiveMongoComponent)(implicit ec: ExecutionContext)
  ...
  override def put(nino: NINO, imtpNeedsUpdate: Boolean): EitherT[Future, String, Unit] = {
    logger.info(s"Putting nino $nino into enrolment store")
    EitherT(
      update(EnrolmentData(nino, imtpNeedsUpdate)).map[Either[String,Unit]]{ result ⇒
...
object MongoEnrolmentStore {
  ...
  private[repositories] case class EnrolmentData(nino: String, imtpHtSFlagSet: Boolean)

  private[repositories]  object EnrolmentData {
